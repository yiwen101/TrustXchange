...
async function get_estimated_ledger(client: Client, date: Date, ledger: LedgerResponse|undefined = undefined): Promise<LedgerResponse> {
    if (!ledger) {
...
export async function get_estimated_ledger_index(client: Client, date: Date, ledger: LedgerResponse|undefined = undefined): Promise<number> {
    const estimated_ledger = await get_estimated_ledger(client, date, ledger);
    return ledger_index(estimated_ledger);
}

export async function get_estimated_ledger_close_time(client: Client, date: Date, ledger: LedgerResponse|undefined = undefined): Promise<Date> {
    const estimated_ledger = await get_estimated_ledger(client, date, ledger);
...
export async function get_xrp_price_at_ledger(ledger_index:number|"validated" = "validated") {
    ...
    try {
        const info = await get_amm_info(client, ledger_index,true);
