...
public class P2PLendingRequestDAO {
    ...
    public void createLendingRequest(P2PLendingRequestDTO request) throws SQLException {
        String sql = "INSERT INTO lending_requests (lender, amount_to_lend_usd, amount_lended_usd, min_collateral_ratio, liquidation_threshold, desired_interest_rate, payment_duration, minimal_partial_fill, canceled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void updateLendingRequest(P2PLendingRequestDTO request) throws SQLException {
        String sql = "UPDATE lending_requests SET lender = ?, amount_to_lend_usd = ?, amount_lended_usd = ?, min_collateral_ratio = ?, liquidation_threshold = ?, desired_interest_rate = ?, payment_duration = ?, minimal_partial_fill = ?, canceled = ? WHERE request_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, request.getLender());
            stmt.setBigDecimal(2, request.getAmountToLendUSD());
            stmt.setBigDecimal(3, request.getAmountLendedUSD());
