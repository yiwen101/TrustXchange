...
public class EventListener {
    ...
    private static final String CONTRACT_ADDRESS = "0x739136918affCEcfd46aBbc417EC815bB6dbB5dd";

    public static void listenFor(int minutes) throws Exception {
        Web3j web3j = Web3j.build(new HttpService(RPC_URL));

        String eventName = "MessageEmitted";
        ...
        for (EthLog.LogResult<?> logResult : ethLog.getLogs()) {
            ...
            String eventHash = web3jLog.getTopics().get(0);
            if (eventHash.equals(MY_EVENT_HASH)) {
                List<?> sth = FunctionReturnDecoder.decode(web3jLog.getData(), event.getNonIndexedParameters());
                Utf8String arg1 = (Utf8String) sth.get(0);
