...
public class P2PBorrowingRequestDAO {
    ...
    public void createBorrowingRequest(P2PBorrowingRequestDTO request) throws SQLException {
        String sql = "INSERT INTO borrowing_requests (request_id, borrower, amount_to_borrow_usd, amount_borrowed_usd, initial_collateral_amount_xrp, existing_collateral_amount_xrp, max_collateral_ratio, liquidation_threshold, desired_interest_rate, payment_duration, minimal_partial_fill, canceled, canceled_by_system) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            stmt.setBoolean(13, request.isCanceledBySystem());
            stmt.executeUpdate();
        }
    }
    ...
    public void updateBorrowingRequest(P2PBorrowingRequestDTO request) throws SQLException {
        String sql = "UPDATE borrowing_requests SET borrower = ?, amount_to_borrow_usd = ?, amount_borrowed_usd = ?, initial_collateral_amount_xrp = ?, existing_collateral_amount_xrp = ?, max_collateral_ratio = ?, liquidation_threshold = ?, desired_interest_rate = ?, payment_duration = ?, minimal_partial_fill = ?, canceled = ?, canceled_by_system = ? WHERE request_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            stmt.setBoolean(12, request.isCanceledBySystem());
